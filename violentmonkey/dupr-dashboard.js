  ...
  function disciplinePanel(type, summary, meta = {}) {
    ...
    return `
      <section class="dupr-panel" data-discipline="${type}">
        <div class="dupr-section-title"><div><span class="dupr-kicker">${type}</span><h2>${summary.games} rated games <small>· ${summary.events} tournaments</small></h2></div><div class="dupr-headline-stats"><div class="dupr-headline-copy"><strong>${summary.currentRating?.toFixed(3) || 'N/A'}</strong><small>${Math.round(summary.winRate * 100)}% win rate</small></div>${reliabilityDonut(meta.reliability)}</div></div>
        ...
        <div class="dupr-charts">
          <figure class="dupr-rating-chart"><figcaption>Rating history</figcaption><canvas data-chart="rating" aria-label="${type} rating history from earliest to latest"></canvas></figure>
        </div>
